...
func (v *BrokerRequest) ReadFrom(r io.Reader) (int64, error) {
	wc := new(writeN)
...
func (v *BrokerRequest) WriteTo(w io.Writer) (int64, error) {
...
func (v *BrokerResponse) ReadFrom(r io.Reader) (int64, error) {
	wc := new(writeN)
	rd := io.TeeReader(r, wc)
	err := json.NewDecoder(rd).Decode(v)
...
func (v *BrokerResponse) WriteTo(w io.Writer) (int64, error) {
	b, err := json.Marshal(v)
	if err != nil {
		return 0, err
	}
...
func (v *BrokerResponse) Error() string {
	if v.Succeed {
		return "<nil>"
	}
